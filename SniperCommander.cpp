...
void SniperCommander::Fight(vector<vector<Soldier *>> &board, pair<int, int> location) {
    dynamic_cast<Sniper *>(board[location.first][location.second])->Fight(board,location);
    int inf = INFINITY;
    int dist = 0;
