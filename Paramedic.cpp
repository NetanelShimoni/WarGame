...
// namespace WarGame
// {
void Paramedic::Fight(vector<vector<Soldier *> > &board, pair<int, int> dest) {
    int iter = 1;
    for (int x = -iter; x <= iter; x++)
    {
        ...
        if (dest.first + iter >= 0 && dest.first + iter < board.size())
        {
            if (dest.second + x >= 0 && dest.second + x < board[dest.first + iter].size())
            {
                ...
                if (s != nullptr && s->get_num_player() == board[dest.first][dest.second]->get_num_player())
                {
                    s->set_health(s->max);
                }
            }
        ...
        if (dest.first - iter >= 0 && dest.first - iter < board.size())
        {
            ...
            {
                s = board[dest.first - iter][dest.second + x];
                if (s != nullptr && s->num_player == board[dest.first][dest.second]->num_player)
                {
                    s->set_health(s->max);
                }
            }
